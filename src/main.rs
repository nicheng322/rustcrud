...
//修改某条记录
#[get("/{id}")]
async fn edit(data: web::Data<AppState>, id: web::Path<i32>/*从请求路径中提取类型化信息 */) -> Result<HttpResponse, Error> {
    let conn = &data.conn;
    let template = &data.templates;
...
async fn main() -> std::io::Result<()> {
    ...
    let port = env::var("PORT").expect("PORT is n+ot set in .env file");
    //格式化服务器地址
